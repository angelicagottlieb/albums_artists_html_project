...
"""
When we call ArtistRepository#findbyname
We get back an error message if it is there and the id if it is found 
"""

def test_get_single_record_with_name(db_connection):
    db_connection.seed("seeds/albums_table.sql")
    repository = ArtistRepository(db_connection)
    result = repository.findbyname("Taylor Swift")
    assert result == 3

def test_get_single_record_with_name_error(db_connection):
    db_connection.seed("seeds/albums_table.sql")
    repository = ArtistRepository(db_connection)
    result = repository.findbyname("Lana Del Rey")
    assert result == "Artist not found"
